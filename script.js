...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check OS preference on initial load
    ...
    if (mobileMenuToggle && navLinks && navOverlay) {
        ...
        navOverlay.addEventListener('click', toggleMenu); // Close when overlay is clicked
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (target) {
                const offsetTop = target.offsetTop - 80; // Adjusted for sticky nav
                window.scrollTo({
                    top: offsetTop,
                    behavior: 'smooth'
                });
    ...
    if (carouselTrack) {
        ...
        let currentIndex = 0;
        const itemWidth = carouselItems[0] ? carouselItems[0].offsetWidth + 32 : 0;
